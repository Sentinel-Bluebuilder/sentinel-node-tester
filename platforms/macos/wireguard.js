// macOS WireGuard tunnel management
...
function findWireGuardExe() {
  // macOS: wireguard-tools via Homebrew or MacPorts
  const paths = [
    '/usr/local/bin/wg',
    '/opt/homebrew/bin/wg',
    '/usr/bin/wg',
...
export function emergencyCleanupSync() {
  ...
  // Also try to find and kill any lingering wg interfaces
  try {
    const ifaces = execSync('ifconfig -l', { encoding: 'utf8', timeout: 5000 }).trim().split(/\s+/);
    for (const iface of ifaces) {
      if (iface.startsWith('utun') || iface.startsWith('wgsent')) {
        try { execSync(`"${WG_QUICK}" down ${iface}`, { timeout: 5000, stdio: 'pipe' }); } catch {}
      }
    }
  } catch {}
...
export async function installWgTunnel(confPath, timeoutMs = 30_000) {
  if (!WG_QUICK) {
    throw new Error('WireGuard not found. Install via: brew install wireguard-tools');
  }
  if (!IS_ADMIN) {
 ...
 * Checks ifconfig/wg show for the interface.
 */
async function verifyTunnelRunning(tunnelName, maxWaitMs = 10000) {
  ...
  while (Date.now() - start < maxWaitMs) {
    ...
    // Also check via ifconfig for utun interfaces (macOS creates utun* for WG)
    try {
      const out = execSync(`ifconfig ${tunnelName} 2>/dev/null || true`, { encoding: 'utf8', timeout: 3000, stdio: 'pipe' });
      if (out.includes('inet')) return true;
    } catch {}
    await sleep(500);
  }
  return false;

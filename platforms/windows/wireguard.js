...
export async function installWgTunnel(confPath) {
  ...
  if (WG_EXE) {
    // Force-stop and remove any leftover tunnel service before installing.
    // Must sc stop BEFORE /uninstalltunnelservice — uninstall silently fails if service is running.
    const svcName = `WireGuardTunnel$${name}`;
    try { execSync(`sc stop "${svcName}"`, { timeout: 5000, stdio: 'pipe' }); } catch { }
    try { runWgCommand(['/uninstalltunnelservice', name], 10_000); } catch { }
    try { execSync(`sc delete "${svcName}"`, { timeout: 5000, stdio: 'pipe' }); } catch { }
    // Verify the service is actually gone before proceeding
    for (let w = 0; w < 5; w++) {
      try {
        const qry = execSync(`sc query "${svcName}"`, { encoding: 'utf8', timeout: 3000, stdio: 'pipe' });
        if (/STOPPED|does not exist|1060/i.test(qry)) break;
      } catch {
        break; // sc query failed = service gone
      }
      await sleep(500);
    }
    await sleep(500);
    ...
    // Verify the service actually reached RUNNING state — don't trust silent success.
    // /installtunnelservice can silently fail if config path is unreadable by SYSTEM.
    ...
    if (!verified) {
      ...
      tunnelInstalledAt = 0;
      throw new Error(
        `WireGuard tunnel '${name}' failed to start — service never reached RUNNING state. ` +
        `Config path: ${confPath}. Ensure app is running as Administrator.`
      );
    }
 ...
 * Polls `sc query` up to 15s. Distinguishes START_PENDING from STOPPED.
 */
...
export async function uninstallWgTunnel(tunnelName) {
  ...
  // Scrub private key before deletion — if unlink fails (file locked), key bytes are zeroed
  try {

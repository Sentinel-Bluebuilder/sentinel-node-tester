...
## 2026-04-26 — Upstream PRs to blue-js-sdk (4/5 opened, PR2 superseded)
- **PR #1 — `pr/country-extras-and-continents`** ([blue-js-sdk#1](https://github.com/Sentinel-Autonomybuilder/blue-js-sdk/pull/1)): COUNTRY_MAP additions (Central Asia + Balkans) + `CONTINENT_BY_CODE` / `CONTINENT_NAMES` / `countryToContinent()` exports in `app-helpers.js`. Pure additive.
- **PR #2 — `pr/node-transport-cache`**: SUPERSEDED. SDK already has equivalent transport-cache surface in `audit.js` (2-tuple keys). Tester's 4-tuple version is a local-only specialization; not worth porting.
- **PR #3 — `pr/post-tunnel-google-checks`** ([blue-js-sdk#2](https://github.com/Sentinel-Autonomybuilder/blue-js-sdk/pull/2)): `resolveGoogleIp()` (cached, 8.8.8.8/1.1.1.1 → system → dns.lookup fallback chain), `checkGoogleDirect()` (IP+SNI HTTPS), `checkGoogleViaSocks5()` (axios + SocksProxyAgent). Appended to top-level `speedtest.js` (canonical). All return `{ googleAccessible, googleLatencyMs, googleError }`.
- **PR #4 — `pr/audit-error-classes`** ([blue-js-sdk#3](https://github.com/Sentinel-Autonomybuilder/blue-js-sdk/pull/3)): `AuditError` base + 6 subclasses (`HandshakeError`, `PaymentError`, `VpnInterferenceError`, `NodeUnreachableError`, `InsufficientBalanceError`, `SpeedTestError`). 5 new `ErrorCodes` constants, 5 new `ERROR_SEVERITY` entries (4 retryable, `VPN_INTERFERENCE`='infrastructure'). Constructor signature `(message, diag = {})`, code hardcoded per subclass; subclass exposes `.diag`. **Skipped** tester's `ChainError`/`TunnelError` because SDK already has those names with a different signature `(code, message, details)` — tester keeps its local versions.
- **PR #5 — `pr/extract-session-map-orphans`** ([blue-js-sdk#4](https://github.com/Sentinel-Autonomybuilder/blue-js-sdk/pull/4)): `extractSessionMap(txResult, nodeAddrs?)` free function appended to `session-manager.js`. Returns `Map<string, bigint>` plus `_orphanIds`/`_needsChainLookup` hint fields for chain-lookup of session_ids that arrive without a colocated `node_address` (most chain heights). Mirrors tester `core/session.js:222`.
- **Mechanics**: 5 git worktrees under `/tmp/blue-js-sdk-pr{1,3,4,5}/` off `blue-js-sdk-base/master`, each on its own branch. Pushed via `gh auth switch --user Sentinel-Autonomybuilder`. Bot identity for commits: `Sentinel-Autonomybuilder@users.noreply.github.com`.
- **Followups (not blocking)**: convert tester's 7 bare `Error` throws to `AuditError` subclasses post-merge of PR #3; `npm pkg fix` on tester; restore `raceStop()` if `/api/stop` latency unacceptable.
